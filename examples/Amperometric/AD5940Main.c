...
void AD5940AMPStructInit(void)
{
  ...
  pAMPCfg->FifoThresh = 4;      		/* Number of measurements before alerting host microcontroller. Must be multiple of 2 */
	
  
	
	/* Configure SE0 Channel */
	pAMPCfg->LptiaRtiaSel = LPTIARTIA_8K;
	pAMPCfg->LpTiaRl = LPTIARLOAD_SHORT;
	
	/* Configure DE0 Channel */
	pAMPCfg->DE0Rload = 0;
	pAMPCfg->DE0Rtia = HSTIARTIA_200;
	
	/* Configure Bias Voltage */
	pAMPCfg->SensorBias = -800;   			/* Sensor bias voltage between reference and sense electrodes*/
	pAMPCfg->Vzero = 1300;        		/* Vzero voltage. Voltage on Sense electrode. Unit is mV*/
